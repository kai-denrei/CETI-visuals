...
| 2026-05-18 | **v4 reuses the v3 `PlaybackClock` without modification.** No `v4-clock.js`. The clockbar element is re-parented above all panes and gated by a `hidden` attribute the router toggles. Tab-switch to v1/v2 calls `clock.pause()` and detaches both v3 and v4 module sets symmetrically. | One clock = one timeline = no race between two ticking subsystems. The pause-on-leave behaviour now applies to both clocked tabs without duplication. | [[dev]] |
| 2026-05-18 | **v4 introduces a single shared simulator `v4-sim.js`**, mirroring the v3 clock's singleton pattern. Three v4 modules (`v4-gauges`, `v4-sonar`, `v4-altimeter`) subscribe to its `subscribe(fn)` API and read fields directly off the snapshot. The sim itself runs on its own RAF (always-on) but only advances values when `clock.playing` is true. | One source of truth means depth on the gauge tile and depth driving the sonar field are exactly equal frame to frame. Letting the sim's RAF stay always-on means canvas resize redraws work even when paused. | [[dev]] |
| 2026-05-18 | **v4 bakes a jet colour LUT (`v4-jet.js`) once at module load.** 256 sRGB triples. Used only by `v4-sonar` and `v4-altimeter`. | A LUT lets the per-pixel inner loop be a single int→3-byte lookup; no closed-form jet math per pixel. The compromise (perceptually uniform rule waived) is bounded to two files. | [[dev]] [[ux]] |
| 2026-05-18 | **v4 ships no pipeline change.** No new JSON, no `bundle.py` edit. The simulator is pure browser-side. | The brief is reconstruction of an operator GUI, not analysis of a corpus. Adding a Python step would couple the v4 visual to the data pipeline for no benefit. | [[dev]] |
...
- 2026-05-18 — v4 tab + four modules added: `web/v4-sim.js` (shared underwater-telemetry simulator), `web/v4-gauges.js` (six-gauge ISD4000 strip), `web/v4-sonar.js` (ISS360 polar), `web/v4-altimeter.js` (ISA500 distance + echogram), `web/v4-jet.js` (baked jet LUT). `app.js` tab router widened to N=4; v3 clockbar re-parented above all panes; `clock.pause()` on leave-to-v1/v2; attach/detach matrix written symmetrically. No `pipeline/` change. No new JSON. Cache-bust token bumped to a185adb0.
