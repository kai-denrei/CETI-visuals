...
| 2026-05-18 | **OVERRIDE FOR v4 — jet colourmap on the ISS360 polar and ISA500 echogram.** The rest of the project bans non-perceptually-uniform colourmaps (`HYDROACOUSTICS.md`). v4 explicitly uses jet because the source instrument does, and the brief is faithful reconstruction. Bounded scope: v4 only. v1/v2/v3 unchanged. | The reader recognises the source software at a glance because of the colour signature; substituting magma would actively defeat the brief. The override is recorded in three places so the rule remains intact everywhere else. | [[arch]] [[pm]] |
| 2026-05-18 | **OVERRIDE FOR v4 — dashboard tile layout.** Six small instrument tiles in a top strip + sonar + altimeter panels below, in a grid. The rest of the project avoids dashboard-template look (`CLAUDE.md`). v4 *is* the dashboard the operator sees; reconstructing it requires the layout. Bounded to v4. | Operators read tile position as data — depth on the far left, temperature next to it, attitude in the middle, turns on the far right. The layout itself is part of the instrument. | [[arch]] [[pm]] |
| 2026-05-18 | **v4 colour palette inherited from the source instrument.** Black sonar background, white tick marks, jet returns, sky-slate / earth-amber attitude horizons, red 7-segment turns counter, amber compass needle. Body chrome remains the editorial dark theme so the tab still belongs to the wider page. | The instrument's identity lives in its own internal palette; the editorial frame is the surround. Mixing them — e.g. teal attitude horizons — would be the worst of both worlds. | [[dev]] |
...
- 2026-05-18 — v4 tab added. Six-gauge instrument strip (depth m+bar, temperature °C with cool/warm tint, compass dial with cardinal letters under amber up-needle, pitch + roll attitude indicators with sky-slate and earth-amber fills, red 7-segment turns counter). ISS360 polar sonar: jet returns, range rings at 0.5/1.0/1.5/2.0 m, deadzone 0.3 m, rotating red sweep with alpha-decay trail. ISA500 altimeter: monospaced distance numeric, traffic-light bar with current-distance cursor, scrolling echogram (jet) with cyan peak-distance line. Clockbar shared with v3. **AESTHETIC OVERRIDE for v4 only** — jet + dashboard, justified by reconstruction brief. Screenshots /tmp/v4-{full,instruments,sonar,altimeter}.png.
